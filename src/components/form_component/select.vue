<template>
	<section class="form-select-container">
		<p class="form-label">
			<span>{{label}}</span>
		...
		<div @click="openSelect()" @mouseleave="stats = false" 
			...
			<div v-if="stats == true" class="relative mx-3">
				<input type="text" class="select-item-search" placeholder="Cari Disini.." v-model="search_bar" >
				<span v-for="(option,index) in options">
					<p v-if="option.toLowerCase().includes(search_bar)" 
					  @click="selectItem(index)" 
					  class="select-item">{{option}}</p>
				</span>
			</div>
		</div>
		<p v-if="footnote" class="text-xs text-gray-500 mt-2" v-html="footnote"/>
	</section>
</template>
...
<style type="text/css">
	...
	.select-item{
		@apply cursor-pointer -mx-3 px-3 text-sm hover:bg-blue-400 mt-1 py-1;
	}
	.select-item-search{
		@apply border mb-1 py-2 w-full mt-2 px-3 bg-gray-100 rounded-md bg-transparent;
	}
</style>